...
//    TestRes reviseRes = userApi.post("/user/books/" + bookId + "/status?cmd=START_REVISION", null);
//    reviseRes.isSuccess();
//
//    // 3. REVISING -> PUBLISHED (cancelRevise)
//    TestRes cancelReviseRes = userApi.post("/user/books/" + bookId + "/status?cmd=CANCEL_REVISION", null);
//    cancelReviseRes.isSuccess();
//
//    // 4. PUBLISHED -> REVISING (다시 revise)
//    TestRes reviseAgainRes = userApi.post("/user/books/" + bookId + "/status?cmd=START_REVISION", null);
//    reviseAgainRes.isSuccess();
//
//    // 5. REVISING -> PUBLISHED (mergeRevision)
//    TestRes mergeRes = userApi.post("/user/books/" + bookId + "/status?cmd=REVISE", null);
//    mergeRes.isSuccess();
//  }
//
//  @Test
...
//    TestRes invalidReviseRes = userApi.post("/user/books/" + bookId + "/status?cmd=START_REVISION", null);
//    invalidReviseRes.is(BookCode.INVALID_BOOK_STATUS);
//
//    // DRAFT 상태에서는 cancelRevise 불가능 (개정 중인 책만 개정 취소 가능)
//    TestRes invalidCancelRes = userApi.post("/user/books/" + bookId + "/status?cmd=CANCEL_REVISION", null);
//    invalidCancelRes.is(BookCode.INVALID_BOOK_STATUS);
//
//    // DRAFT 상태에서는 mergeRevision 불가능 (개정 중인 책만 병합 가능)
//    TestRes invalidMergeRes = userApi.post("/user/books/" + bookId + "/status?cmd=REVISE", null);
...
//    invalidCancelRes.is(BookCode.INVALID_BOOK_STATUS);
//
//    // PUBLISHED 상태에서는 REVISE 불가능 (개정 중인 책만 병합 가능)
//    TestRes invalidMergeRes = userApi.post("/user/books/" + bookId + "/status?cmd=REVISE", null);
//    invalidMergeRes.is(BookCode.INVALID_BOOK_STATUS);
//
...
//    UUID bookId = UUID.fromString(createRes.getData().get("id").asText());
//    userApi.post("/user/books/" + bookId + "/status?cmd=PUBLISH", null).isSuccess();
//    userApi.post("/user/books/" + bookId + "/status?cmd=START_REVISION", null).isSuccess();
//
//    // REVISING 상태에서는 revise 불가능 (이미 개정 중)
//    TestRes duplicateReviseRes = userApi.post("/user/books/" + bookId + "/status?cmd=START_REVISION", null);
...
//    TestRes mergeRevisionRes = userApi.post("/user/books/" + bookId + "/status?cmd=REVISE", null);
...
//    apiTestDataCreator.createUser("user1");
//    apiTestDataCreator.createUser("user2");
