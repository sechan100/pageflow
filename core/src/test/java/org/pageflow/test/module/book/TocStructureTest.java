...
public class TocStructureTest {
  ...
  void tocComplexStructureTest() {
    ...
    Result<FolderDto> folder1Result = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user1.getUid(), rootFolderId, "1장. 서론")
    );
    ...
    Result<FolderDto> folder2Result = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user1.getUid(), rootFolderId, "2장. 본론")
    );
    Assertions.assertTrue(folder2Result.isSuccess());
    UUID folder2Id = folder2Result.getSuccessData().getId();

    Result<FolderDto> folder3Result = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user1.getUid(), rootFolderId, "3장. 결론")
    );
    Assertions.assertTrue(folder3Result.isSuccess());
    UUID folder3Id = folder3Result.getSuccessData().getId();

    // 현재 구조 확인
    toc = tocUseCase.getToc(bookId);
    ...
    Result<SectionDtoWithContent> section1Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), folder1Id, "1.1 서론 개요")
    );
    Assertions.assertTrue(section1Result.isSuccess());
    UUID section1Id = section1Result.getSuccessData().getId();

    // 2장에 섹션 3개 추가
    Result<SectionDtoWithContent> section2Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), folder2Id, "2.1 주제 설명")
    );
    Assertions.assertTrue(section2Result.isSuccess());
    UUID section2Id = section2Result.getSuccessData().getId();

    Result<SectionDtoWithContent> section3Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), folder2Id, "2.2 논점 분석")
    );
    Assertions.assertTrue(section3Result.isSuccess());
    UUID section3Id = section3Result.getSuccessData().getId();

    Result<SectionDtoWithContent> section4Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), folder2Id, "2.3 사례 연구")
    );
    Assertions.assertTrue(section4Result.isSuccess());
    UUID section4Id = section4Result.getSuccessData().getId();

    // 3장에 섹션 1개 추가
    Result<SectionDtoWithContent> section5Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), folder3Id, "3.1 결론 요약")
    );
    ...
    // 5. 중첩 폴더 생성: 2단계
    // 2장 아래에 하위 폴더 생성
    Result<FolderDto> subFolder1Result = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user1.getUid(), folder2Id, "2.A 상세분석")
    );
    Assertions.assertTrue(subFolder1Result.isSuccess());
    UUID subFolder1Id = subFolder1Result.getSuccessData().getId();

    // 하위 폴더에 섹션 추가
    Result<SectionDtoWithContent> section6Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), subFolder1Id, "2.A.1 세부 내용")
    );
    Assertions.assertTrue(section6Result.isSuccess());
    UUID section6Id = section6Result.getSuccessData().getId();

    Result<SectionDtoWithContent> section7Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), subFolder1Id, "2.A.2 추가 분석")
    );
    ...
    // 6. 3단계 중첩 폴더 및 섹션 생성
    Result<FolderDto> subSubFolderResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user1.getUid(), subFolder1Id, "2.A.X 특별 사례")
    );
    Assertions.assertTrue(subSubFolderResult.isSuccess());
    UUID subSubFolderId = subSubFolderResult.getSuccessData().getId();

    Result<SectionDtoWithContent> section8Result = tocUseCase.createSection(
      CreateSectionCmd.withTitle(user1.getUid(), subSubFolderId, "2.A.X.1 특별 케이스")
    );
    ...
    Result moveSection2Result = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user1.getUid())
        .nodeId(section2Id)
        .destFolderId(folder2Id)
        .destIndex(2)
        .build()
    );
    ...
    // 2.A.X.1 섹션을 1장으로 이동
    Result moveSection8Result = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user1.getUid())
        .nodeId(section8Id)
        .destFolderId(folder1Id)
        .destIndex(1)
        .build()
    );
    ...
    // 9. 계층 이동 - 폴더를 상위로 이동
    // 2.A 폴더를 루트로 이동
    Result moveSubFolder1Result = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user1.getUid())
        .nodeId(subFolder1Id)
    ...
    // 3장을 2.A 폴더 안으로 이동
    Result moveFolder3Result = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user1.getUid())
        .nodeId(folder3Id)
        .destFolderId(subFolder1Id)
        .destIndex(0)
        .build()
    );
    ...
    // 섹션 제목 변경
    Result<SectionDto> updateSectionTitleResult = tocUseCase.changeSectionTitle(
      user1.getUid(), section1Id, updatedTitle
    );
    Assertions.assertTrue(updateSectionTitleResult.isSuccess());

    // 섹션 내용 변경
    Result<SectionDtoWithContent> updateSectionContentResult = sectionWriteUseCase.writeContent(
      user1.getUid(), section1Id, updatedContent
    );
    Assertions.assertTrue(updateSectionContentResult.isSuccess());

    // 섹션 조회하여 확인
    Result<SectionDtoWithContent> sectionWithContentResult = sectionWriteUseCase.getSectionWithContent(
      user1.getUid(), section1Id
    );
    ...
    Result<FolderDto> updateFolderResult = tocUseCase.changeFolderTitle(
      user1.getUid(), folder2Id, updatedFolderTitle
    );
    Assertions.assertTrue(updateFolderResult.isSuccess());

    // 폴더 조회하여 확인
    Result<FolderDto> folderResult = tocUseCase.getFolder(user1.getUid(), folder2Id);
    ...
    Result deleteSubFolder1Result = tocUseCase.deleteFolder(user1.getUid(), subFolder1Id);
    ...
    // 14. 섹션 삭제 테스트(2.1)
    Result deleteSection2Result = tocUseCase.deleteSection(user1.getUid(), section2Id);
    ...
    for(int i = 0; i < remainingSections.size(); i++) {
      Result moveSectionResult = tocUseCase.relocateNode(
        RelocateNodeCmd.builder()
          .uid(user1.getUid())
          .nodeId(remainingSections.get(i))
          .destFolderId(rootFolderId)
          .destIndex(i + 2)
          .build()
      );
    ...
    // 16. 남은 모든 폴더 삭제 확인
    Result deleteFolder1Result = tocUseCase.deleteFolder(user1.getUid(), folder1Id);
    Assertions.assertTrue(deleteFolder1Result.isSuccess());

    Result deleteFolder2Result = tocUseCase.deleteFolder(user1.getUid(), folder2Id);
  ...
  void parentToChildMoveTest() {
    ...
    Result<FolderDto> parentFolderResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), rootFolderId, "부모 폴더")
    );
    Assertions.assertTrue(parentFolderResult.isSuccess());
    UUID parentFolderId = parentFolderResult.getSuccessData().getId();

    Result<FolderDto> childFolderResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), parentFolderId, "자식 폴더")
    );
    ...
    Result moveResult = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user.getUid())
        .nodeId(parentFolderId)
        .destFolderId(childFolderId)
        .destIndex(0)
        .build()
    );
  ...
  void parentToGrandchildMoveTest() {
    ...
    Result<FolderDto> grandchildFolderResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), childFolderId, "손자 폴더")
    );
    ...
    Result moveResult = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user.getUid())
        .nodeId(parentFolderId)
        .destFolderId(grandchildFolderId)
        .destIndex(0)
        .build()
    );
  ...
  void rootFolderMoveTest() {
    ...
    // 3. 일반 폴더 생성
    Result<FolderDto> normalFolderResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), rootFolderId, "일반 폴더")
    );
  ...
  void selfParentTest() {
    ...
    // 3. 일반 폴더 생성
    Result<FolderDto> folderResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), rootFolderId, "폴더 A")
    );
    Assertions.assertTrue(folderResult.isSuccess());
  ...
  void complexCyclicReferenceTest() {
    ...
    // 3. 여러 계층의 폴더 구조 생성
    // 루트 > A > B > C > D
    Result<FolderDto> folderAResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), rootFolderId, "폴더 A")
    );
    Assertions.assertTrue(folderAResult.isSuccess());
    UUID folderAId = folderAResult.getSuccessData().getId();

    Result<FolderDto> folderBResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), folderAId, "폴더 B")
    );
    Assertions.assertTrue(folderBResult.isSuccess());
    UUID folderBId = folderBResult.getSuccessData().getId();

    Result<FolderDto> folderCResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), folderBId, "폴더 C")
    );
    Assertions.assertTrue(folderCResult.isSuccess());
    UUID folderCId = folderCResult.getSuccessData().getId();

    Result<FolderDto> folderDResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), folderCId, "폴더 D")
    );
    Assertions.assertTrue(folderDResult.isSuccess());
    UUID folderDId = folderDResult.getSuccessData().getId();

    // 루트 > E > F
    Result<FolderDto> folderEResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), rootFolderId, "폴더 E")
    );
    Assertions.assertTrue(folderEResult.isSuccess());
    UUID folderEId = folderEResult.getSuccessData().getId();

    Result<FolderDto> folderFResult = tocUseCase.createFolder(
      CreateFolderCmd.withTitle(user.getUid(), folderEId, "폴더 F")
    );
    ...
    Result moveResAtoD = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user.getUid())
        .nodeId(folderAId)
        .destFolderId(folderDId)
        .destIndex(0)
        .build()
    );
    ...
    Result moveResEtoF = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user.getUid())
        .nodeId(folderEId)
        .destFolderId(folderFId)
        .destIndex(0)
        .build()
    );
    ...
    Result validMove = tocUseCase.relocateNode(
      RelocateNodeCmd.builder()
        .uid(user.getUid())
        .nodeId(folderFId)
        .destFolderId(folderCId)
        .destIndex(0)
        .build()
    );
