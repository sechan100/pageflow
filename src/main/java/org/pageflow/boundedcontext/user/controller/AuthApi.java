...
import org.pageflow.boundedcontext.user.dto.user.SessionUser;
import org.pageflow.boundedcontext.user.service.AuthService;
import org.pageflow.boundedcontext.user.service.UserValidator;
...
public class AuthApi {
    ...
    public Session getSession() {
        // 사용자 조회
        UserAgg userAggAggregate = userValidator.fetchUser(requestContext.getUID(), UserFetchDepth.FULL);
        // 사용자 정보 객체 생성
        SessionUser userInfo = SessionUser.builder()
                .email(userAggAggregate.getAccount().getEmail())
                .penname(userAggAggregate.getProfile().getPenname())
                .uid(userAggAggregate.getProfile().getId())
                .username(userAggAggregate.getAccount().getUsername())
                .profileImgUrl(userAggAggregate.getProfile().getProfileImgUrl())
                .role(userAggAggregate.getAccount().getRole())
                .isEmailVerified(userAggAggregate.getAccount().isEmailVerified())
                .build();
