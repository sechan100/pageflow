...
public class Username extends SingleValueWrapper<String> {
    // REVIEW: set의 크기가 커지거나, 데이터들을 디스크에 저장할 필요가 있게되면 외부 서비스 로직으로의 분리를 고려
    public static final Set<String> FORBIDDEN_USERNAME_WORDS = Set.of(
//        "admin","administrator","anonymous","pageflow"
    ...
    private static void validate(String username){
        ...
        // 사용할 수 없는 단어 포함여부
        for(String forbiddenWord : FORBIDDEN_USERNAME_WORDS){
            if(username.contains(forbiddenWord)){
                throw Code4.CONTAINS_FORBIDDEN_WORD.feedback(
                    t -> t.getUsername_containsForbiddenWord(forbiddenWord)
                );
            }
        }
    }
