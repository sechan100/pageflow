...
exports.useOutlineMutationStore = void 0;
    ...
    dispatchs: {
        setChapters: function (chapters) {
            set(function (state) { return ({
                payload: __assign(__assign({}, state.payload), { chapters: chapters }),
                isMutated: true
            }); });
        }
    }
...
function BookEntityDraggableContext() {
    ...
    function onDragEnd(result) {
        ...
        if (type === 'CHAPTER' && localOutline.chapters) {
            var newChapters = getReorderedArray(localOutline.chapters, source.index, destination.index);
            ...
            if (sourceChapter === destinationChapter) {
                ...
                var newChapters = localOutline.chapters.map(function (chapter) {
    ...
    function deleteDroppedElement(type, source) {
        if (window.confirm('정말로 삭제하시겠습니까?') === false)
            return;
        // 삭제할 챕터의 경우
        if (type === 'CHAPTER' && localOutline.chapters) {
            var newChapters = Array.from(localOutline.chapters);
            ...
            var newSourcePages = Array.from(sourceChapter_1.pages);
            newSourcePages.splice(source.index, 1);
            sourceChapter_1.pages = newSourcePages;
            var newChapters = localOutline.chapters.map(function (chapter) {
                if (chapter.id === sourceChapter_1.id) {
                    return sourceChapter_1;
                }
                else {
                    return chapter;
                }
    ...
    function getSourceChapterAndDestinationChapter(sourceChapterDroppableId, destinationChapterDroppableId, isInClosingPageDropAreaDropped) {
        ...
        if (isInClosingPageDropAreaDropped) {
            destinationChapter = localOutline.chapters.find(function (chapter) { return Chapter_1.inClosingPageDropAreaPrefix + String(chapter.id) === destinationChapterDroppableId; });
    ...
    function localOutlineReducer(state, action) {
        if (!state.chapters)
            return state;
        switch (action.type) {
            case 'SET_CHAPTERS':
                return __assign(__assign({}, state), { chapters: action.payload });
            case 'SET_PAGES':
                return __assign(__assign({}, state), { chapters: state.chapters.map(function (chapter) {
                        // 변경된 페이지를 가지는 챕터는 새로운 페이지로 교체
                        if (chapter.id === action.chapterId) {
                            return __assign(__assign({}, chapter), { pages: action.payload });
                            // 나머지 페이지는 유지
                        }
                        else {
                            return chapter;
