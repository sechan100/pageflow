...
function MutationSaveBtn() {
    ...
    function flushMutations() {
        if (isAnyMutation) {
            // 변경된 데이터들
            var outlineUpdatePromise = outlineStore.isMutated ? outlineMutateQuery.mutateAsync(outlineStore.payload) : null;
            var bookUpdatePromise = bookStore.isMutated ? bookMutateQuery.mutateAsync(bookStore.payload) : null;
            ...
            Promise.all(updateApiPromises)
                ...
                flowAlert_1["default"]("success", "저장되었습니다.");
    ...
    return (React.createElement("div", { onClick: flushMutations, className: "flex justify-start fixed z-50 right-7 top-7" },
        isAnyMutation &&
