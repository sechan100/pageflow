...
export default function Chapter(props : chapterProps) {
  ...
  return (
    <div className="relative">
      ...
      <Droppable droppableId={inClosingPageDropAreaPrefix + String(chapter.id)} isDropDisabled={!isPagesHidden} type='PAGE'>
        {(provided, snapshot) => (
          <div ref={provided.innerRef} {...provided.droppableProps}>
            <div onMouseOver={toggleChapterFormLink} onMouseOut={toggleChapterFormLink} onClick={toggleInnerPageList} className={(snapshot.isDraggingOver ? "bg-gray-700" : "bg-gray-800") + " flex items-center mt-1 px-1 py-2 w-full text-base font-normal rounded-lg transition duration-75 group border border-[0.1px] border-gray-400 text-white hover:bg-gray-700"}>
              <svg aria-hidden="true" className="w-4 h-4 text-white" fill="none" viewBox="0 0 16 12"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M1 1h14M1 6h14M1 11h7"/></svg>
      ...
      <Link to={"/chapter/" + props.chapter.id} replace onMouseOver={toggleChapterFormLink} onMouseOut={toggleChapterFormLink} className="hidden absolute inline z-50 px-1 py-2 -ml-1 -mt-1 left-[2%] top-1.5 bg-gray-700 items-center text-white text-sm font-normal text-gray-900 rounded-lg transition duration-75 group" ref={chapterFormLink}>
